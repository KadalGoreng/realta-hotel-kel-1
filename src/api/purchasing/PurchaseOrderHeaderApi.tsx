...
const GetData = async (payload: any) => {
  ...
  try {
    const result = await axios.get(`${config.domain}/purchaseOrderHeader?vendorName=${vendorName}&status=${status}&page=${page}`);
    return result.data;
  } catch (error) {
    return error;
  }
...
const Update = async (payload: any) => {
  const { id, ...updatePurchaseOrderHeaderDto } = payload;
  try {
    const result = await axios.put(`${config.domain}/purchaseOrderHeader/${id}`, updatePurchaseOrderHeaderDto);
    return result;
  } catch (error) {
    return error;
  }
