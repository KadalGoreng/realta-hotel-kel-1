import {
  doCardClientRequest,
  restoMenusPhotosUrlRequest,
} from "../../redux/Action/reduceActions";
...
export default function CardRestoMenusClient() {
  ...
  const tambahKeranjang = (cart: any) => {
    const existingCartItem = Keranjang.find(
      (item: any) => item.reme_name === cart.reme_name
    );
        ...
        Keranjang.map((item: any) =>
          item.reme_name === cart.reme_name
            ? {
                ...
                total:
                  parseInt(
                    item.reme_price.split(",")[0].replace(/[^0-9]/g, "")
                  ) *
                  (item.reme_quantity + 1),
              }
            : item
        )
      );
  ...
  const restoMenusCard = useSelector(
    (state: any) => state.restoCardClientReducers.restoCardClient
  );
  ...
  const filteredAndSortedData = Data.filter(
    (item: any) =>
      item.reme_name &&
      item.reme_name.toLowerCase().includes(searchQuery.toLowerCase())
  ).sort((a: any, b: any) => {
                ...
                {filteredAndSortedData.map((CardResto: any) => {
                  return (
                    <div className="card h-auto w-[240px]">
                      ...
                      <div>
                        <h1 className="mt-3 text-xl font-bold text-gray-700">
                          {CardResto.reme_name}
                        </h1>
                        <h1 className="mt-2.5 text-base overflow-auto h-[78px] text-gray-700 hide-scrollbar scrollbar-thumb-red-500  ">
                          {CardResto.reme_description}
                        </h1>
                        <h1
                          className={`mt-3 text-base font-semibold ${
                            CardResto.reme_status === "available"
                              ? "text-green-500"
                              : "text-red-500"
                          }`}
                        >
                          {CardResto.reme_status}
                        </h1>
                        <div className="flex items-baseline justify-between">
                          <div>
                            <h1 className="mt-3 text-lg font-semibold">
                              {CardResto.reme_price}
                            </h1>
                          </div>
                          <div>
                            <h1 className="font-base text-xs text-gray-700">
                              Include Tax
                            </h1>
                          </div>
                        </div>
                        <div className="w-full pt-4">
                          <button
                            className={` rounded h-10 w-full font-reguler  text-white ${
                              CardResto.reme_status === "empty"
              ...
              <div className="card-cart mt-2 h-full w-[350px]">
                      ...
                      >
                        <div>
                          ...
                          <h1 className="mt-1 text-sm font-semibold text-gray-500">
                            ...
                            Rp.
                            {parseInt(
                              nampilinKeranjang.reme_price
                                .split(",")[0]
                                .replace(/[^0-9]/g, "")
                            )}{" "}
                            x {nampilinKeranjang.reme_quantity} = Rp.
                            {nampilinKeranjang.total}
                          </h1>
                        </div>
                        <div>
                          {/* <!-- Hapus --> */}
                          <button
                            className="flex w-full mt-4 rounded-md font-bold text-red-500"
                ...
                <div className="mt-8 w-full">
                  <button
                    ...
                    className="p-3 w-full font-semibold text-center bg-[#F33C5D] text-white rounded "
                  >
                    <a>CREATE ORDER</a>
                  </button>
                </div>
                <Modal footer={null} open={modalIsOpen}>
                  <div>
                    ...
                    <div>
                      <p className=" mt-8 text-center text-2xl font-medium text-green-500">
                        Congrats !{" "}
                      </p>
                      <p className=" mb-6 mt-4 text-center text-sm text-[#4C5563]">
                        Succed Create Order !
                      </p>
                    </div>
                  </div>
