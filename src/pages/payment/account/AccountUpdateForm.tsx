...
export default function AccountUpdateForm(props: any) {
  const dispatch = useDispatch();
  const { account } = useSelector((state: any) => state.accountState);
  useEffect(() => {
    dispatch(FindAccountRequest(props.id));
  }, [dispatch, props.id]);
  const formik = useFormik({
    ...
    initialValues: {
      ...
      exp_month: account.usacExpMonth,
      exp_year: account.usacExpYear,
    },
    onSubmit: async (values) => {
      let payload = {
        ...
        usac_exp_month: values.exp_month,
        usac_exp_year: values.exp_year,
      };
