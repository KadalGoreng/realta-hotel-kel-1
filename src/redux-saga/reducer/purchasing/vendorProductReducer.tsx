...
const VendorProductReducer = (state = INIT_STATE, action: any) => {
  switch (action.type) {
    ...
    case ActionType.FIND_VENDORPRODUCTBYID_REQUEST:
      return { ...state };
    case ActionType.FIND_VENDORPRODUCTBYID_SUCCESS:
      return GetVendorProductByVendorIdSuccessfully(state, action);
    default:
      return { ...state };
  }
};

const GetVendorProductByVendorIdSuccessfully = (state: any, action: any) => {
  console.log(state);
  return {
    ...state,
    vendorProductByVendorId: action.payload,
  };
};
