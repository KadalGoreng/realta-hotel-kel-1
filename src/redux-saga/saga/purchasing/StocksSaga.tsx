...
import StocksApi from "@/api/purchasing/StocksApi";
import { AddStockFailed, AddStockSuccess, DelStockFailed, DelStockSuccess, EditStockFailed, EditStockSuccess, FindStockFailed, FindStockSuccess, GetStockFailed, GetStockSuccess } from "../../action/purchasing/stocksAction";

function* handleStock(action: any): any {
  const { payload } = action;
  try {
    const result = yield call(StocksApi.GetData, payload);
    console.log(result);
