...
import VendorApi from "@/api/purchasing/VendorApi";
import { AddVendorFailed, AddVendorSuccess, DelVendorFailed, DelVendorSuccess, EditVendorFailed, EditVendorSuccess, FindVendorFailed, FindVendorSuccess, GetVendorFailed, GetVendorSuccess } from "../../action/purchasing/vendorAction";

function* handleVendor(action: any): any {
  const { payload } = action;
  try {
    const result = yield call(VendorApi.GetData, payload);
    console.log(result);
